...
double getAltitude(int fd) { // Returns the pressure altitude in feet
	double pressure;
//	bmp_GetTemperature(
	bmp_GetPressure(fd, &pressure);
	return bmp_altitude(pressure);
}
int bmpSetup(){
	int fd = i2c_Open(I2CBus);
	bmp_Calibration(fd);
	double temp;
	bmp_GetTemperature(fd, &temp); //without grabbing the temperature, the bmp180 wont return a correct altitude
	return fd;
}
